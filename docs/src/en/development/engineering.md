...
| `just docs-serve` | Build both languages once and serve locally |
| `just docs-serve-dirty` | Watch and rebuild both languages; refresh the browser |
| `just docs-build` | Build the static documentation site |
| `just examples` | pVisor product example suite |
| `just dev` | Format, lint, and Rust tests |
...
For supported behavior, start with [pVisor Guides](../guides/index.md)
and consult [System Design](../design/index.md) for the rationale behind
an implementation.
