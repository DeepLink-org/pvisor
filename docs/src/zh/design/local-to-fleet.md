...
!!! note "设计方向"
    ...
    本页描述未来部署模型。当前 CLI 提供本地 executor，不提供集群调度器、租约服务或远程证明系统。

可移植单位是逻辑 Run，而不是一台正在运行的虚拟机。

![PolicyVisor 执行连续体](../../assets/diagrams/agentvisor/execution-continuum.svg)
...
稳定 identity 模型见 [Run、Attempt 与 Effect](../concepts/run-model.md)。Provider
admission 属于 [pVisor 隔离设计](isolation.md)；集群协调（placement、lease
