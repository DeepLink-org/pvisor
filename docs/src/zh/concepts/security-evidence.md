...
PolicyVisor 不会把安全压缩成一个 `safe` 或 `sandboxed` 标签。每个 Run 都按 capability
...
| 凭据 | 显式环境变量交付 | 哪个 identity 获得并使用 Secret？ |
| Effect | 暂存、应用或丢弃记录 | 哪些后果进入真实系统？ |

以下分级用于描述证据声明，不代表所有 executor 均已实现；Attested 不表示当前提供密码学远程证明服务。
...
用户模型见 [Capability 与 Evidence](capabilities-and-evidence.md)，平台机制见
[pVisor 隔离设计](../design/isolation.md)与 [OverlayNet](../design/overlaynet.md)。
